...
export default defineConfig({
  ...
  server: {
    ...
    port: 5173,
    // Accept the Host header when reached over a tunnel/proxy (e.g. a *.ts.net
    // Tailscale hostname). Without this, Vite blocks the request as an
    // unrecognized host. LAN access by IP:port is unaffected.
    allowedHosts: ['.ts.net'],
