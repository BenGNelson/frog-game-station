"""
Library — the games hub's pure logic (no FastAPI here, so it's all unit-tested).

The Library is one section per content type. Each section reads from a content
dir and recognizes a set of file extensions. This build ships the **games**
section (ROMs → EmulatorJS); more sections would slot in by adding a SECTION
entry + a dir setting, with no router changes.
...
def sections_summary(settings):
    ...
    for s in SECTIONS:
        ...
        preview = [it["id"] for it in items[:_PREVIEW_COUNT] if it.get("id")]
        out.append(
            {
                "key": s["key"],
                "label": s["label"],
