...
- **In-game wiki reader** — pull up a game's wiki *over the paused game* (a controller hotkey or the pause menu), read a type chart or a hack's changed movesets, then close and reopen right where you left off. It's a reader, not an iframe: the article is fetched from the wiki's own API, sanitized, and rendered in-app — so it's fully controller- and touch-navigable and matches the theme. Links come from IGDB automatically; anything unmatched (a ROM hack) gets a one-tap **search-and-pin**, and a hack's search even defaults to the right wiki by family (a Pokémon hack → Bulbapedia).
